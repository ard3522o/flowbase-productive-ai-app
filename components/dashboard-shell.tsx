...
export function DashboardShell() {
  ...
  useEffect(() => {
    fetch("/api/dashboard").then(r => r.json()).then(d => {
      if (!d.error) setDashData(d);
    }).catch(() => {});
  }, []);

  const stats = dashData ? [
    { label: "Boards", value: String(dashData.stats.boards), detail: dashData.stats.boards === 0 ? "Create your first board" : "Active boards", icon: Columns3, tone: "bg-violet-100 text-violet-600" },
    { label: "Tasks", value: dashData.stats.tasksDone + " / " + dashData.stats.tasks, detail: dashData.stats.tasks === 0 ? "No tasks yet" : dashData.stats.tasksDone + " completed", icon: CheckCircle2, tone: "bg-emerald-100 text-emerald-600" },
    { label: "Notes", value: String(dashData.stats.notes), detail: dashData.stats.notes === 0 ? "Start writing" : dashData.stats.notes + " notes", icon: NotebookPen, tone: "bg-sky-100 text-sky-600" },
    { label: "Spaces", value: String(dashData.stats.spaces), detail: dashData.stats.whiteboards + " whiteboards", icon: Files, tone: "bg-amber-100 text-amber-600" },
  ] : [
  ...
  return <main className="flex min-h-screen bg-[#fbfcff] text-slate-900">
    <aside className={`sticky top-0 flex h-screen shrink-0 flex-col border-r border-slate-200/80 bg-white/95 px-2.5 py-3 backdrop-blur transition-[width] duration-300 ${collapsed ? "w-[68px]" : "w-[228px]"}`}>
      ...
      <div className={"border-t border-slate-100 pt-3 " + (collapsed ? "" : "px-1")}>
        {!collapsed && <p className="mb-1.5 text-[9px] font-bold uppercase tracking-[0.14em] text-slate-400">Account</p>}
        <div className={"flex w-full items-center rounded-lg p-1 " + (collapsed ? "justify-center" : "gap-2")}>
          <UserButton />
          {!collapsed && <span className="min-w-0 flex-1"><p className="block truncate text-[11px] font-semibold text-slate-700">My Account</p><p className="block truncate text-[9px] text-slate-400">Manage settings</p></span>}
        </div>
      </div>
    </aside>
