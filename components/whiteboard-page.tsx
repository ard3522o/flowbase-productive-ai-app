    ...
    <main className="flex min-h-screen bg-[#fbfcff] text-slate-900">
      ...
      <div className={"flex h-screen shrink-0 flex-col border-r border-slate-200/80 bg-white transition-all duration-300 overflow-hidden " + (sidebarOpen ? "w-[300px]" : "w-0 border-0")}>
        ...
        <div className="flex-1 overflow-y-auto px-2 py-1 space-y-0.5">
          {filtered.map((b) => {
            ...
            return (
              <div key={b.id} onClick={() => setActiveId(b.id)} className={"group relative flex cursor-pointer items-start gap-2.5 rounded-xl px-3 py-2.5 transition-all " + (isActive ? "ring-1 shadow-sm ring-orange-200 bg-orange-50" : "hover:bg-slate-50:bg-slate-800")}>
        ...
        {active ? (
          <div className="flex items-center justify-between border-b border-slate-200 px-4 py-2 bg-white z-10">
            ...
            <div className="flex items-center gap-2">
              ...
              <button type="button" onClick={exportPNG} className="inline-flex items-center gap-1.5 rounded-lg border border-slate-200 bg-white px-3 py-1.5 text-[11px] font-semibold text-slate-600 hover:bg-slate-50:bg-slate-800 transition-colors"><Download className="h-3 w-3" /> Export PNG</button>
            </div>
          </div>
        ) : null}
      ...
      {ctxMenu && (
        <div data-ctx style={{ position: "fixed", top: ctxMenu.y, left: ctxMenu.x, zIndex: 60 }} className="w-[180px] rounded-xl border border-slate-200 bg-white p-1.5 shadow-xl">
          ...
          <button type="button" onClick={() => { setColorPicker(ctxMenu.id); setCtxMenu(null); }} className="flex w-full items-center gap-2.5 rounded-lg px-3 py-2 text-[12px] font-medium text-slate-600 hover:bg-slate-50:bg-slate-800 transition-colors">Color</button>
